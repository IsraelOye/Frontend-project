...
const Blog = () => {
  return (
    <div>
      <Navbar />
      <div className="px-4 sm:px-6 md:px-8 lg:px-12 xl:px-6 py-6 md:py-8 lg:py-10 pt-20 sm:pt-24 md:pt-32 lg:pt-36 xl:pt-40 bg-gray-50 min-h-screen">
        <h1 className="text-2xl sm:text-3xl md:text-4xl text-center font-bold text-gray-800 mb-6 md:mb-8 px-4">
          Our Blogs
        </h1>
            ...
            >
              <div className="relative overflow-hidden">
                <img
                  src={blog.image}
                  alt={blog.title}
                  className="h-40 sm:h-44 md:h-48 w-full object-cover transition-transform duration-300 hover:scale-105"
                />
              </div>
              <div className="p-4 md:p-5">
                <h2 className="text-base sm:text-lg font-semibold text-gray-800 mb-2 line-clamp-2 leading-tight">
                  {blog.title}
                </h2>
                <p className="text-xs sm:text-sm text-gray-500 mb-3 md:mb-4">
                  {blog.date} {blog.author && `• ${blog.author}`}
                </p>
                <p className="text-sm sm:text-base text-gray-700 mb-3 md:mb-4 line-clamp-3 leading-relaxed">
                  {blog.excerpt}
                </p>
                <button className="text-sm sm:text-base text-blue-600 hover:text-blue-800 hover:underline transition-colors duration-200 font-medium">
                  Read More →
                </button>
              </div>
            </div>
